...
```bash
npm run build:web
npm run dev -- web --path /duong/dan/toi/project
```

- `web` tự tạo snapshot nếu chưa có state.
- `web` khởi động server local và in URL giao diện.
- Web UI hỗ trợ nhiều project trong cùng một phiên.
- Web UI tự làm mới khi file thay đổi.
- Web UI cho phép rollback và reset baseline.

## Giao diện web

- Tabs để chuyển giữa nhiều project.
- Bộ lọc `all`, `added`, `modified`, `deleted`.
- Nút `Refresh`, `Rollback`, `Reset baseline`.
- Panel trái là danh sách file thay đổi.
- Panel phải là diff file đang chọn.
